...
public partial class wfmServiceRequest : System.Web.UI.Page
{
    String strDocType;
    String strDocNo;
    ...
    private void ValidateUserInformation(DataTable dtTable)
    {
        if (dtTable.Rows.Count > 0)
        {
            ...
            cmdAttachments.Visible = cmdSave.Visible;
        }
    }
    ...
    private void PopulateHeaderInformation(DataTable dtTable)
    {
        if (dtTable.Rows.Count > 0)
        {
            ...
            ddLStatus.Text = dtRow["SERVICE_STATUS"].ToString();
            txtDetails.Text = dtRow["DETAILS"].ToString();
            txtExpectedDate.Text = dtRow["EXPECTED_DATE"].ToString();
            txtProblemType.Text = dtRow["PROBLEM_TYPE"].ToString();
            ...
            ddlReAllocatedTo.Text = dtRow["RE_ALLOCATEDTO"].ToString();
            txtRemarks.Text = dtRow["REMARKS"].ToString();
            txtSolutionDetails.Text = dtRow["DESC_ABOUT_SOLUTION"].ToString();
            chkEmail.Checked = PublicMethods.ToInt16(dtRow["IN_EMAIL"]) == 1 ? true : false;
            txtCCList.Text = dtRow["CC_LIST"].ToString();
        }
    }
        ...
        if (Session["UserLogin"] != null)
        {
            ...
            if (CheckData() == true)
            {
                ...
                catch (SqlException ex)
                {
                    ...
                    if (ex.Message.Contains("PRIMARY KEY constraint 'PK_" + "ITNET_SERVICE_REQUEST" + "'") == true)
                    {
    ...
    private void SaveAll(DataLayer dlDataLayer, String dtCurrentDateTime)
    {
        ...
        sqlObj.TableName = "ITNET_SERVICE_REQUEST";
        if (strAddEditDeleteStatus == "A")
        {
        ...
        sqlObj.AddColumns("SERVICE_STATUS", ddLStatus.Text, PublicVariables.DataType.Text);
        ...
        sqlObj.AddColumns("PROBLEM_TYPE", txtProblemType.Text, PublicVariables.DataType.Text);
        ...
        sqlObj.AddColumns("RE_ALLOCATEDTO", ddlReAllocatedTo.Text, PublicVariables.DataType.Text);
        sqlObj.AddColumns("DESC_ABOUT_SOLUTION", txtSolutionDetails.Text, PublicVariables.DataType.Text);
        sqlObj.AddColumns("IN_EMAIL", chkEmail.Checked == true ? 1 : 0, PublicVariables.DataType.Number);
        sqlObj.AddColumns("CC_LIST", txtCCList.Text, PublicVariables.DataType.Text);
